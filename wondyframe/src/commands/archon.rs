...
pub async fn archon(
    ...
    let archon: Archon = match warframe_client.fetch::<Archon>("archonHunt").await {
        Ok(v) => v,
        Err(e) => {
            ...
            ctx.send(
                poise::CreateReply::default()
                    .content("Could not fetch Archon data at this time due to external failure with Warframe API.")
                    .ephemeral(!is_public),
            )
            .await?;
